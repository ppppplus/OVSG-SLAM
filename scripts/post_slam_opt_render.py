...
def rgbd_slam(config: dict):
    ...
    eval_params = dict(np.load(ckpt_path, allow_pickle=True))
    eval_params = convert_store_to_params(eval_params)
    ...
    # Mapping
    print("Evaluating Params at 15K Iterations")
    for time_idx in tqdm(range(num_frames)):
        # Get RGB-D Data & Camera Parameters
        if load_semantics:
            color, depth, intrinsics, pose, semantic_id, semantic_color = eval_dataset[time_idx]
            semantic_id = semantic_id.permute(2, 0, 1) # (H, W, 1) -> (1, H, W)
            semantic_color = semantic_color.permute(2, 0, 1) # (H, W, C) -> (C, H, W)
        else:
            color, depth, intrinsics, pose = eval_dataset[time_idx]
        ...
        if load_semantics:
            ...
            semantic_rendervar = semantics2rendervar(eval_params)
            rastered_seg, _, _, = Renderer(raster_settings=cam)(**semantic_rendervar)
            gt_seg = semantic_color
            viz_rastered_seg = (rastered_seg + 1) / 2.0
            viz_seg = (gt_seg + 1) / 2.0
            viz_rastered_seg = torch.clip(viz_rastered_seg, 0, 1)
            viz_seg = torch.clip(viz_seg, 0, 1)

            img1 = viz_seg.permute(1, 2, 0).cpu().numpy()  # -> (H, W, 3)
            img2 = viz_rastered_seg.permute(1, 2, 0).detach().cpu().numpy()

            fig, axes = plt.subplots(1, 2, figsize=(12, 6))
            axes[0].imshow(img1)
            axes[0].set_title("viz_seg")
            axes[0].axis('off')

            axes[1].imshow(img2)
            axes[1].set_title("raster_seg")
            axes[1].axis('off')
