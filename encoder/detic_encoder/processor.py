...
class DeticProcessor:
    ...
    def __init__(self, detic_config, vocabulary, custom_vocabulary):
        ...
        self.predictor = VisualizationDemo(detectron_cfg, dummy_args, meta_dir=CURRENT_DIR)   # define detic predictor
    ...
    def infer(self, image) -> InferenceRawResult:
        ...
        featuredim = features[0].shape[0]
        featuremap = torch.zeros((featuredim, img.shape[0]*img.shape[1]))
        for i, mask in enumerate(pred_masks):
            featuremap[:, mask.flatten()] = features[i][:,None]
        featuremap = featuremap.view(featuredim, img.shape[0], img.shape[1])
