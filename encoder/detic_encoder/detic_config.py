...
sys.path.insert(0, os.path.join(CURRENT_DIR, 'third_party/CenterNet2/'))
...
class DeticConfig:
    def __init__(self, args):
        ...
        self.detic_config_path = os.path.join(
            self.current_directory, 'detic_configs',
            self.model_name + '.yaml')

        self.model_weights_path = os.path.join(
            self.current_directory, 'models',
            self.model_name + '.pth')
    ...
    def to_detectron_config(self):
        ...
        cfg.MODEL.ROI_BOX_HEAD.CAT_FREQ_PATH = os.path.join(
            self.current_directory, 'metadata/lvis_v1_train_cat_info.json')
